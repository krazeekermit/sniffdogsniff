...
"""
from sdsjsonrpc import connect, config, history
from sdsjsonrpc.server import Server
from sdsjsonrpc import logger
from sdsjsonrpc.errors import ProtocolError, EncryptionMissing
...
class SerializableDummyObj:
    def __init__(self, **kwargs):
        self._name = kwargs['name']
        self._surname = kwargs['surname']
        self._age = int(kwargs['age'])
...
def get_dummy_person():
    return SerializableDummyObj(name='Michael', surname='Spencer', age=24)
...
def test_set_up():
    ...
    server.serializer.register_object(SerializableDummyObj, ['name', 'surname', 'age'])
    server.add_handler(get_dummy_person, 'get_dummy_person')
...
class TestSerialization(unittest.TestCase):
    def setUp(self) -> None:
        ser = JsonSerializer()
        ser.register_object(SerializableDummyObj, ['name', 'surname', 'age'])
        self._serializer = ser

    def test_serializer(self):
        dummy = SerializableDummyObj(name='Michael', surname='Spencer', age=24)
        dummy_dict = {
            'person': dummy,
            'description': 'over the people state lives, under the state people dies',
            'dict': {'n': ['1', '100', 1002], 't': 'tag1'}
        }
        text = self._serializer.dumps(dummy_dict)
        des_dict = self._serializer.loads(text)
        self.assertEqual(des_dict['description'], 'over the people state lives, under the state people dies')
        self.assertIsInstance(des_dict['person'], SerializableDummyObj)
        self.assertEqual(des_dict['person'].name, 'Michael')
        self.assertEqual(des_dict['person'].surname, 'Spencer')
        self.assertEqual(des_dict['person'].age, 24)
        self.assertEquals(des_dict['dict'], {'n': ['1', '100', 1002], 't': 'tag1'})

    def test_serialization_client_server(self):
        response = CLIENT.get_dummy_person()
        self.assertIsInstance(response, SerializableDummyObj)
